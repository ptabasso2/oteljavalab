...
When examining the trace, you might have noticed that the service is actually named after the name of the artifact (`springtotel-0.0.1-snapshot`). Which is probably what we would like to see in a production environment.
...
If we run the application by providing the service name, we can see that the two methods are being instrumented are correlated with the previous span that was generated by the the java agent and correctly named after the desired service name 
